$(function () {
	...
	$('#myTab a[href="#addTab"]').on('click', function () { // Click event on the "Add Tab" button
		...
		var FirstPart = '<div = class="row"><div class="form-group">'
		FirstPart = FirstPart + '<div> Background Processing<font id="read"> </font></div><div><label id="DataFactoryProcessing" class="label-bullet">Not Processing</label></br>';
		FirstPart = FirstPart + '<img id="DataProcessingGIF" src="Gif/gears7.gif" />';
		FirstPart = FirstPart + '</div></div>';
		var SecondPart = '<div class="col-lg-12"><div class="col-md-2 col-sm-2 col-xs-2"><label class="label-bullet-blue">Select File Type</label>';
		SecondPart = SecondPart + '<select id="sel_generic_uploader_file_type_' + nbrLiElem + '" onchange="generic_uploader_reset_table_comboboxes()" class="form-control"></select>';
		SecondPart = SecondPart + '</div><div id="CustomerSelectorDiv" class="col-md-4 col-sm-4 col-xs-4" hidden="true">';
		SecondPart = SecondPart + '<label class="label-bullet-blue">Customer</label>';
		SecondPart = SecondPart + '<select id="cboCustomerSelector_' + nbrLiElem + '" class="form-control">';
		SecondPart = SecondPart + '<option value="0">- Customer -</option></select>';
		SecondPart = SecondPart + '</div><div class="col-md-2 col-sm-2 col-xs-2"><label class="label-bullet-blue">Start Line of Data</label>';
		SecondPart = SecondPart + '<select id="LineLength_' + nbrLiElem  + '" class="form-control"></select></div></div>';

		var FinalPart = "<div/>"
		$('div.tab-content div:last-child').after('<div class="tab-pane fade" id="tab' + (nbrLiElem + 1) + '"><p> Content tab ' + (nbrLiElem + 1) + '</p>' + FirstPart + SecondPart + FinalPart + '</div>');
		GeneralSelFill('sel_generic_uploader_file_type_' + nbrLiElem, '- File Type -', '/Services/CurveUploader.svc/FileTypeGetInfo');
		GeneralSelFill('LineLength_' + nbrLiElem, '- Line Length -', '/Services/CurveUploader.svc/LineStartGetInfo');
