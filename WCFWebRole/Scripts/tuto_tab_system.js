$(function () {
	...
	$('#myTab a[href="#addTab"]').on('click', function () { // Click event on the "Add Tab" button
		...
		var element = '<input type="button" id="validate-button_tabs' + (nbrLiElem + 1) + '" class="btn btn-primary btn-lg" value="Import Into Table ' + (nbrLiElem + 1) + '" onclick="DisplayActiveTabl()" />';
		
		$('div.tab-content div:last-child').after('<div class="tab-pane fade" id="tab' + (nbrLiElem + 1) + '"><p>Content JUST MORE tab ' + (nbrLiElem + 1) + '</p>' + element);
		//$('div.tab-content div:last-child').after('<div class="tab-pane fade" id="tab' + (nbrLiElem + 1) + '"><p>Content tab ' + (nbrLiElem + 1) + '</p>');
